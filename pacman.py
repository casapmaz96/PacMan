...
def parseAgentArgs(str):
    ...
    for p in pieces:
        if '=' in p:
            ...
            key,val = p, 1
        if key=='epsilon' or key=='alpha' or key=='gamma':
            opts[key] = float(val)
        else:
            opts[key] = val
#    print(opts)
...
#    parser.add_option('-t', '--textGraphics', type='str2bool', dest='textGraphics',
    ...
    parser.add_option('-w', dest='width', type='int',
                      help=default('Width of the board'), default=7)
    parser.add_option('--height', dest='height', type='int',
                      help=default('Height of the board'), default=7)
   # parser.add_option('--resume', dest='resume',
    #                  help='Resume training with saved model parameters', default=True)
