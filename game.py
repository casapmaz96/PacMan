    ...
    def run( self ):
        ...
        while not self.gameOver:
            ...
            if 'observationFunction' in dir( agent ):
                ...
                if self.catchExceptions:
                    ...
                    #print('agent.observationFunction is called with state: ', self.state, '\nfor agent: ', agentIndex)
                    observation = agent.observationFunction(self.state.deepCopy())
                self.unmute()
            else:
                #print('observationFunction not found in dir(agent), state: ', self.state, '\nfor agent: ', agentIndex)
            ...
            if self.catchExceptions:
                try:
                    #print('successor being generated for state after catchException: ', self.state, '\n')
                    self.state = self.state.generateSuccessor( agentIndex, action )
                    #print('successor state: ', self.state, '\n')
                except Exception as data:
                    self.mute(agentIndex)
                    self._agentCrash(agentIndex)
                    self.unmute()
                ...
                #print('successor being generated for state without catchException: ', self.state, '\n')
                self.state = self.state.generateSuccessor( agentIndex, action )
                #print('successor state: ', self.state, '\n')
            # Change the display
        ...
        for agentIndex, agent in enumerate(self.agents):
            if "final" in dir( agent ) :
                #print('final state executing\n')
                try:
                    self.mute(agentIndex)
                    #print('agent ', agentIndex, 's final state: ', self.state, '\n')
